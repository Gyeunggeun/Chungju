...
def datahandling() :
    ...
    # streamlit//data_subway_in_seoul.csv
    # encoding='cp949'  읽어오고 확인하기 
    df = pd.read_csv('D:\Python01\DX_데이터분석표현\실습자료\스트림릿_실습자료\data_subway_in_seoul.csv',encoding='cp949')
    st.dataframe(df)
    ...
    df_line_melted3 = df_line_melted2.pivot_table(index='시간', columns='호선', values='인원수', aggfunc='sum')
